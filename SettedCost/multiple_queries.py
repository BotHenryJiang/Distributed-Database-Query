...
def evaluate_optimizer(query_router, queries, optimizer):
    performance_metrics = {
        "Optimization Time": 0,
        "Query Execution Time": 0
    }
    
    with open(f"{type(optimizer).__name__}_cost.csv", "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Optimizer", "Optimized Query", "Cost"])
        
        for query in queries:
            results, query_times, total_query_time, optimization_time = query_router.query(query, optimizer)
            performance_metrics["Optimization Time"] += optimization_time
            performance_metrics["Query Execution Time"] += total_query_time
            writer.writerow([type(optimizer).__name__, query, total_query_time])
    
    performance_metrics["Optimization Time"] /= len(queries)
    performance_metrics["Query Execution Time"] /= len(queries)
...
class GeneticAlgorithmOptimizer(Optimizer):
    ...
    def optimize(self, query):
        ...
        def fitness(individual):
            # 使用查询执行时间作为适应度函数
            try:
                individual_query = " and ".join(str(elem) for elem in individual)
                ...
                print(f"Query failed: {' and '.join(str(elem) for elem in individual)}, Error: {e}")
                return float('inf')

        def crossover(parent1, parent2):
            if len(parent1) <= 2 or len(parent2) <= 2:
                # 如果父节点长度小于等于2,则返回原有的父节点
        ...
        def mutate(individual):
            ...
            # 交换两个随机选择的元素
            idx1 = random.randint(0, len(individual_list) - 1)
            idx2 = random.randint(0, len(individual_list) - 1)
...
optimizers = {
    ...
    #GreedyOptimizer(),
    #GANOptimizer(all_data),
    GeneticAlgorithmOptimizer(all_data) #传递 all_data 给遗传算法优化器
...
queries = [
    "id < 100 and category == 'A'",
    "data.str.contains('S') and category == 'A' and data.str.contains('X') and id >=500000 and id<700000",
    "category == 'B' and data.str.contains('AFG')",
    "id >= 75 and category == 'C' and data.str.contains('AFG')",
    "id >= 150 and id <750000 and data.str.contains('G') and category == 'F' and id <=650000 and data.str.contains('AB')"
]



for optimizer in optimizers:
    evaluate_optimizer(QueryRouter(shard_manager), queries, optimizer)