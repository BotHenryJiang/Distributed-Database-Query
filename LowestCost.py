...
def find_lowest_cost_path(data_name):
    """
    查找数据，并计算最低成本的传输方案。
    """
    locations = []  # 存放数据所在节点
    ...
    for node, data in nodes.items():
        if data_name in data:
            ...
            total_size += size
            # 寻找最低成本路径
            if node != 'node0':  # 如果数据不在node0上
                # 直接传输成本
                direct_cost, direct_path = calculate_cost(node, 'node0', size)
                min_cost = direct_cost
                min_path = direct_path
                # 通过其他节点作为中转站传输的成本
                for intermediate_node in nodes.keys():
                    if intermediate_node not in [node, 'node0']:
                        to_intermediate_cost, to_intermediate_path = calculate_cost(node, intermediate_node, size)
                        from_intermediate_cost, from_intermediate_path = calculate_cost(intermediate_node, 'node0', size)
                        intermediate_total_cost = to_intermediate_cost + from_intermediate_cost
                        if intermediate_total_cost < min_cost:
                            min_cost = intermediate_total_cost
                            min_path = f"{node} -> {intermediate_node} -> node0"
                total_cost += min_cost
                transfer_details.append((min_path, min_cost))
...
# 示例查找
data_name = 'data6'  # 想要查找的数据名称
result, locations, total_cost, transfer_details = find_lowest_cost_path(data_name)
print(result)
